import RestoCard, { vegCard } from "./RestoCard";
...
const Body = () => {
  ...
  //hoc
  const RestoVegCard = vegCard(RestoCard);
  ...
  if (onlineStatus === false) {
    return <h1>Looks like you have a bad internet connection</h1>;
  }
  console.log(restoList[0]?.info);
    ...
    <div className="body">
      <div className="filter flex">
        <div className="search m-4 p-4">
          ...
          />
          <button
            className="search-btn bg-green-100 px-4 py-1 m-4 rounded-lg cursor-pointer"
            onClick={() => {
              // console.log(searchText)
              // console.log(restoList[0].info.name)
              const searchResto = restoList.filter((resto) =>
        ...
        <div className="search m-4 p-4 flex items-center">
          <button
            ...
            onClick={() => {
              const filterdList = restoList.filter(
